...
df['Age and Gender'] = df['Age'].astype(str) + " " + df['Gender']
...
# Multi-select filter
age_gender_options = df['Age and Gender'].unique()
selected_age_gender = st.multiselect("Select Age and Gender", age_gender_options)

# Filter the dataframe based on selection
if selected_age_gender:
    filtered_df = df[df['Age and Gender'].isin(selected_age_gender)].copy()
else:
...
filtered_df = filtered_df[['Age and Gender', 'Event', 'Marshall Area', 'Status']]
